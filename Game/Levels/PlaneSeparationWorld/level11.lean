...
Statement auxiliary_point (hAm : A ∈ m) (hAs : A ∉ ℓ) :
  ...
  · intro hEm
    ...
    · rw [between_symmetric] at hDAE
      have hD : D ∈ line_through E A := mem_line_through_of_between hDAE
      rw [line_through_symmetric] at hD
      rw [← hm] at hD
      exact hD
    tauto
  -- Prove that same_side ℓ A E
  · have hA : A ∈ line_through D E := mem_line_through_of_between' hDAE
    ...
    · intro x hx hAxE
      ...
      have hxD : x = D
      · apply equal_points_of_in_two_lines hℓs (by exact mem_line_through_of_between' hAxE) hx _ hDℓ
        rw [between_symmetric] at hDAE
        rw [line_through_symmetric]
        exact mem_line_through_of_between hDAE
      rw [hxD] at hAxE
